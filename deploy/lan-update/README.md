...
确认更新后卡住或窗口消失时，将 diagnose_client.bat 和 diagnose-client.ps1
放在故障电脑的软件 EXE 所在目录，双击 diagnose_client.bat，拍摄最后的
PHOTO THIS SUMMARY 区域。无需安装 Python，无需外网，也无需管理员权限。
诊断只读取清单和更新包前四个字节，并在临时目录用固定的无更新清单测试更新器；
不会安装更新、关闭业务软件或改写更新配置。会暂存一份运行库，可能耗时数分钟。
RUNTIME_OLD_LAYOUT 与 RUNTIME_WITH_ROOT_DLLS 用于对比临时目录缺少根目录 DLL 的影响。
HTTP 探测分别使用系统默认代理和直连；它不能完全代表 Python 更新器的代理行为。
RESULT.txt 和临时运行库保留在结果中的 REPORT 目录，诊断结束后可手动删除该目录。

若诊断显示旧布局启动失败、补齐根目录 DLL 后正常，可以使用一次性更新入口：
将 start_update_with_runtime.bat 和 start-update-with-runtime.ps1 放到软件 EXE 旁，
先正常关闭两个软件（包括托盘中的程序），再双击 BAT。它会读取现有版本和更新地址，
复制完整运行库与根目录 DLL，在隔离目录启动现有更新器，执行正常的哈希校验和更新。
它不修改更新地址，不自动结束业务进程，也不改写服务器。遇到更新锁时停止，不删除锁。
成功后手动打开软件确认版本；失败时拍摄控制台错误，并保留显示的诊断目录及 UPDATE_LOG.txt。
此入口不是安装到旧客户端的永久补丁；若服务器发布的版本尚未修复按钮，请保留此工具。
3.2.25 已修复根目录 DLL 漏复制、准备阶段界面阻塞及未确认启动就退出的问题。
受影响的 3.2.23/3.2.24 客户端应先使用上述一次性入口升级到 3.2.25，之后恢复内置更新入口。
...
- 确认更新后没有更新窗口：新版客户端会保留主窗口并提示启动失败，查看软件目录的
  UPDATE_STARTUP_LOG.txt；目录不可写时查看 %TEMP%\PythonVNA_UPDATE_STARTUP_LOG.txt。
  UPDATE_LOG.txt 在更新器进入主函数后记录启动阶段；不存在该日志不代表服务器部署失败。
  更新器的临时运行目录必须同时包含 EXE 旁的 DLL 和完整 _internal，不能只复制 EXE。
