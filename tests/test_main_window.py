...
class MainWindowTests(unittest.TestCase):
    ...
    def test_live_plot_refresh_reuses_curve_items_and_updates_data(self):
        measurement = self._measurement()
        self.controller.state.measurement = measurement
        self.window.top_display_combo.setCurrentText("time")
        self.window._plot_measurement(measurement)
        original_curve = self.window._plot_curve_items["top"]["ai0"]

        refreshed = self._measurement()
        refreshed.time_data["channels"]["ai0"] = np.array(
            [4.0, 3.0, 2.0, 1.0], dtype=float
        )
        self.window._plot_measurement(refreshed)
        refreshed_curve = self.window._plot_curve_items["top"]["ai0"]
    ...
    def test_live_plot_refresh_removes_stale_curve_items(self):
        ...
        self.window.top_display_combo.setCurrentText("time")
        self.window._plot_measurement(measurement)
        stale_curve = self.window._plot_curve_items["top"]["ai1"]

        refreshed = self._measurement()
        refreshed.time_data["channels"].pop("ai1")
        self.window._plot_measurement(refreshed)

        self.assertNotIn("ai1", self.window._plot_curve_items["top"])
        self.assertNotIn(stale_curve, self.window.top_plot.listDataItems())
