...
HIDDEN += collect_submodules("python_samba.logging_tools")
HIDDEN += collect_submodules("python_samba.ui")

a = Analysis(
    [str(ROOT / "packaging" / "entries" / "entry_samba.py")],
    pathex=[str(SAMBA / "src")],
    binaries=[],
...
pyz = PYZ(a.pure)
exe = EXE(
    pyz, a.scripts, [], exclude_binaries=True, name="Samba",
    debug=False, bootloader_ignore_signals=False, strip=False, upx=False,
    console=False, disable_windowed_traceback=False, argv_emulation=False,
    target_arch=None, codesign_identity=None, entitlements_file=None,
)
coll = COLLECT(
    exe, a.binaries, a.datas, strip=False, upx=False, upx_exclude=[], name="Samba"
)
